"""Scripted audit evidence for post-open mq:* label removal tampering."""
...
SCENARIO_ID = "audit_label_removed"
EXPECTED: dict[str, Any] = {
    "action": "Eject",
    "family": "label tamper",
    "detail": "post-open-remove",
...
_FILE_PATH = "projects/hipdnn/audit-evidence-label-removed.txt"
_FILE_CONTENT = "audit evidence: post-open mq label remove\n"
_LABEL_TO_REMOVE = "mq:hipdnn"
...
def _prepare_pr(client: Any, owner: str, repo: str) -> tuple[int, str, str]:
    return create_audit_pr(
        ...
        title_suffix="post-open-label-remove",
    )
...
def _tamper(client: Any, owner: str, repo: str, pr_number: int) -> dict[str, Any]:
    ...
    client.rest.issues.remove_label(owner, repo, pr_number, _LABEL_TO_REMOVE)
    return {
        "operation": "remove_label",
        "label": _LABEL_TO_REMOVE,
