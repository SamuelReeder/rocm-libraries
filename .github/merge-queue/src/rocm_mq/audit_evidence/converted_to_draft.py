"""Scripted audit evidence for converting an enqueued PR to draft."""
...
_FILE_PATH = "projects/hipdnn/audit-evidence-converted-to-draft.txt"
_FILE_CONTENT = "audit evidence: converted to draft\n"
...
def _prepare_pr(client: Any, owner: str, repo: str) -> tuple[int, str, str]:
    return create_audit_pr(
        ...
        title_suffix="converted-to-draft",
    )
...
def _tamper(client: Any, owner: str, repo: str, pr_number: int) -> dict[str, Any]:
    ...
    observed = run_gh_pr_ready_undo(owner, repo, pr_number)
    observed["labels_before_tamper"] = list(labels_before)
    observed["merge_command_comment_id"] = command_id
    return observed
