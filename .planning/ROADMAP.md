...
**Goal**: Every row of the RFC §4.3.1 tamper matrix demonstrably triggers the documented response on the fork; self-bootstrap protection rejects every adversarial path-intersection variant; `PATH_TO_QUEUES` config edits without graph closure fail the validator.
**Depends on**: Phase 3
**Requirements**: AUDIT-01, AUDIT-02, AUDIT-03, AUDIT-04, MGD-01, MGD-02, MGD-03, VAL-01, VAL-02, VAL-03, VAL-04
**Success Criteria** (what must be TRUE):
  ...
  2. A CI lint enforces zero `actions/checkout` in any workflow that runs `pull_request_target` (`mq-handler.yml`, `mq-managed-status.yml`); the lint fails the build if any future PR adds one. This defends Pitfall 1 — the single highest-impact bug class in the design surface.
  3. `mq-managed-status.yml` posts `merge-queue/managed` on every PR's head SHA — `pending` for opted-in paths, `success` for non-opted-in (so required-check semantics treat missing as blocking) — and the processor flips opted-in PRs from `pending` to `success` immediately before squash. Verified against fork PRs touching opted-in paths and PRs touching non-opted-in paths.
  4. `mq-config-validate.yml` rejects every adversarial `PATH_TO_QUEUES` edit: missing-closure (downstream queue added but upstream entry not updated), self-bootstrap path-intersection bypass attempts (symlinks under `.github/`, case-only renames, generated-file regeneration paths). Property tests pin the rejection set.
  5. `PATH_TO_QUEUES` is loaded via the GitHub Contents API at `ref=develop` for handler and processor (`load_from_develop`); `load_local` is reachable only from the validator workflow. Unit + integration tests confirm a PR cannot modify its own queue routing by editing the config in its own branch.
**Plans**: TBD

### Phase 5: Porting Prep
**Goal**: A future implementer (or the upstream reviewer) can land this implementation on `ROCm/rocm-libraries` cleanly using the artifacts produced — runbook for operations, pre-flight checklist for upstream-side actions, RFC tweaks log for design round-tripping, and a squash-clean branch ready to PR.
**Depends on**: Phase 4
